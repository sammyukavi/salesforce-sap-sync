...
public class CustomerDbService extends BaseDbDataService<Customer> {
	
	@Override
	protected Class<Customer> getDaoServiceClass() {
		return Customer.class;
	}
	
	
	public void getUnsynced(GetCallback<ArrayList<Customer>> callback) {
		try {
			...
			QueryBuilder<User, Integer> userQueryBuilder =
					(QueryBuilder<User, Integer>) DaoManager.createDao(connectionSource, User.class).queryBuilder();
			
			queryBuilder.leftJoin(userQueryBuilder);
			
			callback.onCompleted((ArrayList<Customer>) dao.query(queryBuilder.prepare()));
	...
	public void upsertRecords(ArrayList<Customer> customers, GetCallback<ArrayList<Customer>> callback) {
		...
		try {
			...
			TransactionManager.callInTransaction(connectionSource, () -> {
				...
				callback.onCompleted(upsertedCustomers);
				return null;
			});
		} catch (SQLException e) {
			callback.onError(e);
